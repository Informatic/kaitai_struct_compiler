...
class TypeScriptTranslator(provider: TypeProvider) extends BaseTranslator(provider) {
  ...
  /**
    * TypeScript rendition of common control character that would use hex form,
  ...
  override def doInternalName(id: Identifier): String =
    s"(this.${TypeScriptCompiler.publicMemberName(id)} as any)"
  ...
  /**
    ...
    * TypeScript. There are quite a few methods to so, this one is generally
  ...
  /**
    * Converts a float to an integer in TypeScript. There are many methods to
    ...
    * @see https://developer.mozilla.org/en/docs/Web/TypeScript/Reference/Global_Objects/Math/trunc
